        ...
        if model_arch in [
            ...
            if is_deepseek_nsa(hf_config):  # DeepSeek 3.2/GLM 5
                if model_arch == "GlmMoeDsaForCausalLM" and is_blackwell_supported():
                    ...
                    if envs.SGLANG_NSA_DENSE_ATTN_KV_LEN_THRESHOLD.is_set():
                        logger.warning(
                            f"Dense attention kv len threshold is manually set to {envs.SGLANG_NSA_DENSE_ATTN_KV_LEN_THRESHOLD.get()} for DSA. Caution: This may cause performance regression if the threshold is larger than the index topk of model."
                        )
                    else:
                    ...
                    if self.nsa_prefill_backend == "trtllm":
                        logger.warning(
                            "TRTLLM sparse MLA kernel requires MHA as prefill impl, the threshold for dense attention is overridden."
                        )
